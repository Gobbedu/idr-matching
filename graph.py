...
def gen_graph(img):
    if DEBUGPRINT: print("shape: %s  |  max: %d  |  min: %d" % (img.shape, img.max(), img.min()))
    ...
    if DEBUGPRINT: print('vertex pixels: %d' % (len(naive_vertexes)))
...
def merge_vertexes(vertexes):
    ...
    if DEBUGPRINT: print("detected %d merges to execute: %s" % (len(vertexes_to_merge), vertexes_to_merge))
...
def remove_isolated_vertexes(vertexes):
    ...
    if DEBUGPRINT: print("post-processing 2: remove isolated vertexes (<=%d neighbor)" % (ISO_NEIGH))
    ...
    while vertexes_to_check:
        ...
        if DEBUGPRINT: print("detected %d vertexes to remove: %s" % (len(vertexes_to_remove), vertexes_to_remove))
...
def graph(img_path):
    """Heart"""
    # print(f'\rinitializing {img_path}...', end=' ')
    # print(f'initializing {img_path}...')
